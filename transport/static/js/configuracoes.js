...
document.addEventListener('DOMContentLoaded', function() {
    // Check if we're on a page with CT-e elements
    const isCTePage = document.getElementById('cte-list') !== null;
    ...
    if (isCTePage) {
        ...
        // This is likely the configurations page, only set up basic event listeners
        setupConfigurationEventListeners();
    }
});

/**
 * Sets up event listeners for configuration page
 */
function setupConfigurationEventListeners() {
    // Add any configuration-specific event listeners here
    console.log('Configuration page loaded');
    
    // Example: Setup form submission handlers for configuration forms
...
function setDefaultDateRange() {
    ...
    // Format dates for input fields (YYYY-MM-DD) - only if elements exist
    const dataInicioEl = document.getElementById('data_inicio');
    const dataFimEl = document.getElementById('data_fim');
    
    if (dataInicioEl) {
        dataInicioEl.value = formatDateForInput(thirtyDaysAgo);
    }
    if (dataFimEl) {
        dataFimEl.value = formatDateForInput(today);
    }
}
