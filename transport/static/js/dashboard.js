...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Define período padrão inicial (mês atual)
    const periodoSelect = document.getElementById('periodo');
    if (periodoSelect) {
        periodoSelect.value = 'mes'; // Define 'Este Mês' como padrão inicial
        handlePeriodChange('mes'); // Aplica o range de data para o mês atual
    } else {
        setDefaultDateRangeForMonth(); // Fallback
    }
...
function resetFiltersAndLoad() {
    ...
    const periodoSelect = document.getElementById('periodo');
    if (periodoSelect) {
        periodoSelect.value = 'mes'; // Define 'mes' como padrão
        handlePeriodChange('mes'); // Atualiza as datas e desabilita
    } else {
        setDefaultDateRangeForMonth(); // Fallback
    }
...
/**
 * Sets default date range (current month) - Fallback
 */
function setDefaultDateRangeForMonth() {
    const today = new Date();
    ...
    if(dataInicioInput) dataInicioInput.value = formatDateForInput(firstDayOfMonth);
    if(dataFimInput) dataFimInput.value = formatDateForInput(lastDayOfMonth);
}
