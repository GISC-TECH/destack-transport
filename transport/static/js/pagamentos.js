...
async function loadPagamentosAgregados() {
    try {
        ...
        const data = await window.apiClient.get(`/api/pagamentos/agregados/?${queryString}`);
...
async function updateSummaryAgregados() {
    try {
        ...
        const data = await window.apiClient.get(`/api/pagamentos/agregados/?${queryString}&page_size=1000`);
        if (!data) return;
...
async function loadPagamentosProprios() {
    try {
        ...
        const queryString = new URLSearchParams(params).toString();
        const data = await window.apiClient.get(`/api/pagamentos/proprios/?${queryString}`);
...
async function updateSummaryProprios() {
    try {
        ...
        const data = await window.apiClient.get(`/api/pagamentos/proprios/?${queryString}&page_size=1000`);
        if (!data) return;
        const pagamentos = data.results || [];
...
async function loadFaixasKM() {
    try {
        const data = await window.apiClient.get('/api/faixas-km/');
...
async function confirmarGerarAgregados() {
    try {
        ...
        const result = await window.apiClient.post('/api/pagamentos/agregados/gerar/', payload);
...
async function confirmarGerarProprios() {
    try {
        ...
        const result = await window.apiClient.post('/api/pagamentos/proprios/gerar/', payload);
...
async function salvarFaixa() {
    try {
        ...
        if (method === 'POST') {
            await window.apiClient.post(url, payload);
        } else {
            await window.apiClient.put(url, payload);
        }
...
async function exportarAgregados() {
    try {
        ...
        const response = await window.apiClient.request('GET', `/api/pagamentos/agregados/export/?${queryString}`);
...
async function exportarProprios() {
    try {
        ...
        const response = await window.apiClient.request('GET', `/api/pagamentos/proprios/export/?${queryString}`);
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
...
async function editarPagamento(id, tipo) {
    try {
        ...
        const pagamento = await window.apiClient.get(`/api/pagamentos/${endpoint}/${id}/`);
...
async function salvarEdicaoPagamento() {
    try {
        ...
        const endpoint = tipo === 'agregado' ? 'agregados' : 'proprios';
        await window.apiClient.patch(`/api/pagamentos/${endpoint}/${id}/`, payload);
...
async function excluirPagamento(id, tipo) {
    ...
    try {
        const endpoint = tipo === 'agregado' ? 'agregados' : 'proprios';
        await window.apiClient.delete(`/api/pagamentos/${endpoint}/${id}/`);
...
async function editarFaixa(id) {
    try {
        const faixa = await window.apiClient.get(`/api/faixas-km/${id}/`);
...
async function excluirFaixa(id) {
    ...
    try {
        await window.apiClient.delete(`/api/faixas-km/${id}/`);
...
async function loadVeiculosSelect() {
    try {
        const data = await window.apiClient.get('/api/veiculos/?ativo=true&tipo_proprietario=00');
        if (!data) return;
